...
import { getBackgroundColor  } from "src/settings/defaultColors";
...
const getTabInfo = async () => {
  try {
    ...
    const tabUrl = browser.tabs.sendMessage(tab.id, {
      message: "getTabUrl" 
    });
    ...
    return {
      isConnected: false, url: "", selectedText: "", isEnabledOnPage: false 
    };
  }
};
...
export default class PopupPage extends Component {
  ...
  toggleEnabledOnPage = async e => {
    ...
    this.setState({
      isEnabledOnPage: isEnabled 
    });
    try {
      const tab = (await browser.tabs.query({
        currentWindow: true, active: true 
      }))[0];
      if (isEnabled) { await browser.tabs.sendMessage(tab.id, {
        message: "enableExtension" 
      }); }
      else { await browser.tabs.sendMessage(tab.id, {
        message: "disableExtension" 
      }); }
    } catch (e) { }
  };
